...
class DataGrid extends Component {
  ...
  componentWillReceiveProps(nextProps) {
    ...
    const contentWidth =
      this.mainGrid._leftGridWidth + this.mainGrid._bottomRightGrid._scrollingContainer.scrollWidth;
  ...
  renderMultiGrid({ width, height }) {
    ...
    const boxShadow = this.state.scrolledAllLeft ? false : '1px 3px 3px #a2a2a2';
  ...
  _renderFooter() {
    const { pageSize, totalItemCount, currentPage, onUpdateDataNeeded } = this.props;

    const showing = Math.min(totalItemCount, (currentPage - 1) * pageSize + pageSize);
    return (
      <div className="grid-footer">
        <div className="paginator">
          <div className="paginator-label">
            <em>
              Showing {(currentPage - 1) * pageSize + 1} - {showing} of {totalItemCount}
            </em>
          </div>
          <Pagination
            size="small"
